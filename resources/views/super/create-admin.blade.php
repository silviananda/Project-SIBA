		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
					...
					<form role="form" action="{{ route('data-admin.store') }}" method="post">
					@csrf
						...
						<select name="kode_ps" class="form-control @error('kode_ps') is-invalid @enderror">
							<option value="">Pilih Jurusan</option>
								@foreach ($jurusan as $data)
									<option value="{{ $data->jurusan }}" {{ old('kode_ps') == $data->jurusan ? 'selected' : null }} >{{ $data->nama_jurusan}}</option>
								@endforeach
						</select>
							@error('kode_ps')
								<div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="name" value="{{ old ('name') }}">
