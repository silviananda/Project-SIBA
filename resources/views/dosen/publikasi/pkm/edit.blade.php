        ...
        <div class="col-md-12 col-sm-12">
            <div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
                <div class="x_title">
                    ...
                    <h2>Form Ubah Data Pkm</h2>
                    <ul class="nav navbar-right panel_toolbox"></ul>
                    <div class="clearfix"></div>
                </div>

                <form role="form" action="{{ route('publikasi.pkm.update', $data_pkm->id) }}" method="post" enctype="multipart/form-data">
                    @method('patch')
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('tema') is-invalid @enderror" placeholder="" name="tema" value="{{ $data_pkm->tema }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('judul_pkm') is-invalid @enderror" placeholder="" name="judul_pkm" value="{{ $data_pkm->judul_pkm }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <span class="input-group-btn">
                                            <button data-room="{{ $data_pkm->id ?? null }}" data-mhs_id="{{ $item->biodata_mhs->id ?? null }}" class="btn btn-danger" type="button" onclick="remove_mahasiswa_fields_edit({{$item->biodata_mhs->id ?? null}});">
                    ...
                    <div class="form-group">
                        ...
                        <select name="tahun" class="form-control @error('tahun') is-invalid @enderror">
                            ...
                            @for ($year = date('Y') - 7; $year < date('Y') + 10; $year++) <option value="{{$year}}" {{$year == $data_pkm->tahun ? 'selected': ''}}>{{$year}}</option>
                                @endfor
                        </select>
                        @error('tahun')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group">
                        <label for="exampleFormControlInput1">Bukti Pengabdian Kepada Masyarakat</label>
                        <input type="file" class="form-control-file" id="softcopy" name="softcopy" value="{{ $data_pkm->softcopy }}">
                    </div>

                    {{-- untuk verifikasi --}}
                    <div class="form-group">
                        <label>
                            <input type="checkbox" name="is_verification" class="js-switch" {{ $data_pkm->is_verification ? 'checked' : ' ' }} style="display: none;"> Verifikasi
                        </label>
                    </div>

                    <button type="submit" class="btn btn-primary" style="float: right;">Simpan</button>
                    <a href="{{ route('publikasi.pkm.index') }}" class="btn btn-danger" style="float: right;">Batal</a>
