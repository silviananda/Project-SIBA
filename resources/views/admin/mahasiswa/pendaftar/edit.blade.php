		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Ubah Data Pendaftar</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('mahasiswa.daftar.update', $pendaftar->id) }}" method="POST">
					@method('patch')
					@csrf
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('no_ujian') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="no_ujian" value="{{ $pendaftar->no_ujian }}">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('nama') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="nama" value="{{ $pendaftar->nama }}">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('asal_sekolah') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="asal_sekolah" value="{{ $pendaftar->asal_sekolah }}">
						...
						<select name="jalur_masuk_id" class="form-control @error('jalur_masuk_id') is-invalid @enderror">
							<option value="">Pilih Jalur Masuk</option>
								@foreach ($kategori_jalur as $data)
									<option value="{{ $data->id }}" {{$data->id == $pendaftar->jalur_masuk_id ? 'selected' : ''}} >{{ $data->jalur_masuk}}</option>
								@endforeach
						</select>
                            ...
                            <label for="exampleFormControlInput1">Tahun Masuk<span class="danger" style="color: #DC143C;">*</span></label>
                                ...
                                <option value="">Pilih Tahun Masuk</option>
                                    @for ($year = date('Y') - 7; $year < date('Y') + 10; $year++)
                                        <option value="{{$year}}" {{$year == $pendaftar->tahun_masuk ? 'selected': ''}}>{{$year}}</option>
                                    @endfor
                               </select>
                        </div>

                        <button type="submit" class="btn btn-primary" style="float: right;">Simpan</button>
                        <a href="{{ route('mahasiswa.daftar.index') }}" class="btn btn-danger" style="float: right;">Batal</a>
