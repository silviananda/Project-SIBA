		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Tambah Data Pendaftar</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('mahasiswa.daftar.store') }}" method="post">
					@csrf
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('no_ujian') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="no_ujian" value="{{ old ('no_ujian') }}">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('nama') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="nama" value="">
						...
						<div class="form-group">
                            <label for="exampleFormControlInput1">Tahun Masuk<span class="danger" style="color: #DC143C;">*</span></label>
                                <select name="tahun_masuk" class="form-control  @error('jalur_masuk_id') is-invalid @enderror">
                                <option value="">Pilih Tahun Masuk</option>
                                    @for ($year = date('Y') - 7; $year < date('Y') + 10; $year++)
                                        <option value="{{$year}}" {{ old('tahun_masuk') == $pendaftar ? 'selected': ''}}>{{$year}}</option>
                                    @endfor
                               </select>
