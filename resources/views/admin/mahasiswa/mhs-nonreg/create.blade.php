		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Tambah Data Mahasiswa Non Reguler</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

				<form role="form" action="{{ route('mahasiswa.non-reguler.store') }}" method="post">
					@csrf
						...
						<select name="id_status" class="form-control @error('id_status') is-invalid @enderror" disabled>
							<option value="">Pilih Status Mahasiswa</option>
								...
								<option value="{{ $data->id }}" {{ $data->id == 2 ? 'selected' : '' }}>{{ $data->status}}</option>
								@endforeach
						</select>
						@error('id_status')
						<div class="invalid-feedback">{{ $message }}</div>
