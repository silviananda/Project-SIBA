		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Tambah Prestasi Mahasiswa</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('mahasiswa.prestasi.store') }}" method="post" enctype="multipart/form-data">
					@csrf
						...
						<select name="kategori_id" class="form-control @error('kategori_id') is-invalid @enderror">
							<option value="">Pilih Kategori Prestasi</option>
								@foreach ($kategori_data as $data)
									<option value="{{ $data->id }}" {{ old('kategori_id') == $data->id ? 'selected' : null }} >{{ $data->nama}}</option>
								@endforeach
						</select>
							@error('kategori_id')
								<div class="invalid-feedback">{{ $message }}</div>
