	...
	<div class="">
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    <h2>Form Ubah Data Tenaga Kependidikan</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

                    <form role="form" action="{{ route('tenaga-kependidikan.update', $tenaga_kependidikan->id) }}" method="post">
                        @method('patch')
                        @csrf
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('nidn') is-invalid @enderror" id="nidn" placeholder="" name="nidn" value="{{ $tenaga_kependidikan->nidn }}">
                            @error('nidn')
								<div class="invalid-feedback">{{ $message }}</div>
							@enderror
                        </div>
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" placeholder="" name="nama" value="{{ $tenaga_kependidikan->nama }}">
                            @error('nama')
								<div class="invalid-feedback">{{ $message }}</div>
							@enderror
                        </div>
                        ...
                        <div class="form-group">
                            <label for="exampleFormControlInput1">Tanggal Lahir<span class="danger" style="color: #DC143C;">*</span></label>
                                <input id="tgl_lahir" class="date-picker form-control" placeholder="dd-mm-yyyy" type="text" onfocus="this.type='date'" onclick="this.type='date'" onblur="this.type='text'" onmouseout="timeFunctionLong(this)" name="tgl_lahir" value="{{ $tenaga_kependidikan->tgl_lahir }}">
                                    <script>
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('alamat') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="alamat" value="{{ $tenaga_kependidikan->alamat }}">
						...
						<select name="unit_kerja_id" class="form-control @error('unit_kerja_id') is-invalid @enderror">
							<option value="{{ $tenaga_kependidikan->unit_kerja_id }}">Pilih Unit Kerja</option>
								@foreach ($data_unit_kerja as $data)
									<option value="{{ $data->id }}" {{$data->id == $tenaga_kependidikan->unit_kerja_id ? 'selected' : ''}} >{{ $data->unit}}</option>
								@endforeach
						</select>
							@error('unit')
								<div class="invalid-feedback">{{ $message }}</div>
						...
						<select name="data_tenaga_kependidikan_id" class="form-control @error('data_tenaga_kependidikan_id') is-invalid @enderror">
							<option value="{{ $tenaga_kependidikan->data_tenaga_kependidikan_id }}">Pilih Profesi</option>
								@foreach ($data_tenaga_kependidikan as $data)
									<option value="{{ $data->id }}" {{$data->id == $tenaga_kependidikan->data_tenaga_kependidikan_id ? 'selected' : ''}} >{{ $data->posisi}}</option>
								@endforeach
						</select>
						...
						<select name="pendidikan_id" class="form-control @error('pendidikan_id') is-invalid @enderror">
							<option value="{{ $tenaga_kependidikan->pendidikan_id }}">Pilih Pendidikan Terakhir</option>
								@foreach ($daftar_pendidikan as $data)
									<option value="{{ $data->id }}" {{$data->id == $tenaga_kependidikan->pendidikan_id ? 'selected' : ''}} >{{ $data->nama_pendidikan}}</option>
								@endforeach
						</select>
							@error('pendidikan_id')
								<div class="invalid-feedback">{{ $message }}</div>
