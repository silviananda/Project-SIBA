		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
                    ...
                    <form role="form" action="{{ route('upaya.rekognisi.update', $rekognisi->id) }}" method="post">
                        @method('patch')
                        @csrf
						...
						<div class="form-group">
								...
								<input type="text" oninput="autofill('dosen-tetap')" class="dosen-tetap form-control @error('nip') is-invalid @enderror" id="nip" placeholder="Masukkan NIP Dosen" name="nip" value="{{ $rekognisi->dosen['nip'] }}">
						...
						<div class="form-group">
							...
							<input type="text" class="dosen-tetap form-control @error('nama_dosen') is-invalid @enderror" disabled="disabled" id="nama_dosen" placeholder="" name="nama_dosen" value="{{ $rekognisi->dosen['nama_dosen'] }}">
						</div>

						<div class="form-group form-material row">
							<input type="hidden" class="dosen-tetap form-control @error('dosen_id') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="dosen_id" value="{{ $rekognisi->dosen_id }}">
						</div>

						<div class="form-group">
							<label for="exampleFormControlInput1">Bidang Keahlian<span class="danger" style="color: #DC143C;">*</span></label>
							<input type="text" class="form-control @error('bidang_keahlian') is-invalid @enderror" placeholder="" name="bidang_keahlian" value="{{ $rekognisi->bidang_keahlian }}">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('rekognisi') is-invalid @enderror" placeholder="" name="rekognisi" value="{{ $rekognisi->rekognisi }}">
                            ...
                            <label for="exampleFormControlInput1">Tahun Rekognisi<span class="danger" style="color: #DC143C;">*</span></label>
                                ...
                                <option value="">Pilih Tahun Rekognisi</option>
                                    @for ($year = date('Y') - 7; $year < date('Y') + 10; $year++)
                                        <option value="{{$year}}" {{$year == $rekognisi->tahun ? 'selected': ''}}>{{$year}}</option>
                                    @endfor
                               </select>
							@error('tahun')
								<div class="invalid-feedback">{{ $message }}</div>
