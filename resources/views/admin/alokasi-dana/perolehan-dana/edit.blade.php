		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Ubah Data Perolehan Dana</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('alokasi-dana.perolehan.update', $jenis_dana->id) }}" method="post">
					@method('patch')
					@csrf
					...
					<div class="form-group">
						...
						<select name="sumber_dana_id" class="form-control @error('sumber_dana_id') is-invalid @enderror">
							<option value="">Pilih Sumber Dana</option>
								@foreach ($sumber_dana as $data)
									<option value="{{ $data->id }}" {{$data->id == $jenis_dana->sumber_dana_id ? 'selected' : ''}} >{{ $data->nama_sumber_dana}}</option>
								@endforeach
						</select>
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('nama_jenis_dana') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="nama_jenis_dana" value="{{ $jenis_dana->nama_jenis_dana }}">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('jumlah_dana') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="jumlah_dana" value="{{ $jenis_dana->jumlah_dana }}">
                            ...
                            <label for="exampleFormControlInput1">Tahun Perolehan Dana</label>
                                ...
                                <option value="">Pilih Tahun</option>
                                    @for ($year = date('Y') - 7; $year < date('Y') + 10; $year++)
                                        <option value="{{$year}}" {{$year == $jenis_dana->tahun ? 'selected': ''}}>{{$year}}</option>
                                    @endfor
                               </select>
