		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Ubah Data Penggunaan Dana</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('alokasi-dana.penggunaan.update', $penggunaan_dana->id) }}" method="post">
					@method('patch')
					@csrf
						...
						<select name="kategori_pengelola_id" class="form-control @error('kategori_pengelola_id') is-invalid @enderror">
							<option value="">Pilih Kategori Pengelola</option>
								@foreach ($kategori_pengelola as $data)
									<option value="{{ $data->id }}" {{$data->id == $penggunaan_dana->kategori_pengelola_id ? 'selected' : ''}} >{{ $data->pengelola}}</option>
								@endforeach
						</select>
                        ...
                        <label for="exampleFormControlInput1">Kategori Jenis Penggunaan<span class="danger" style="color: #DC143C;">*</span></label>
						<select name="jenis_penggunaan_id" class="form-control @error('jenis_penggunaan_id') is-invalid @enderror">
							<option value="">Pilih Kategori Penggunaan</option>
								@foreach ($jenis_penggunaan as $data)
									<option value="{{ $data->id }}" {{$data->id == $penggunaan_dana->jenis_penggunaan_id ? 'selected' : ''}} >{{ $data->nama_jenis_penggunaan }}</option>
								@endforeach
						</select>
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('dana') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="dana" value="{{ $penggunaan_dana->dana }}">
                            ...
                            <label for="exampleFormControlInput1">Tahun Penggunaan Dana<span class="danger" style="color: #DC143C;">*</span></label>
                                ...
                                <option value="">Pilih Tahun</option>
                                    @for ($year = date('Y') - 7; $year < date('Y') + 10; $year++)
                                        <option value="{{$year}}" {{$year == $penggunaan_dana->tahun ? 'selected': ''}}>{{$year}}</option>
                                    @endfor
                               </select>
