		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Ubah Data Prasarana</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('prasarana.data.update', $data_prasarana->id) }}" method="post">
					@method('patch')
					@csrf
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('jenis_prasarana') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="jenis_prasarana" value="{{ $data_prasarana->jenis_prasarana }}">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('total_luas') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="total_luas" value="{{ $data_prasarana->total_luas }}">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control @error('jumlah_unit') is-invalid @enderror" id="exampleFormControlInput1" placeholder="" name="jumlah_unit" value="{{ $data_prasarana->jumlah_unit }}">
						...
						<select name="kepemilikan" class="form-control @error('kepemilikan') is-invalid @enderror">
							<option value="">Pilih Jenis</option>
								@foreach ($kategori_kepemilikan as $data)
									<option value="{{ $data->id }}" {{$data->id == $data_prasarana->kepemilikan ? 'selected' : ''}} >{{ $data->jenis}}</option>
								@endforeach
						</select>
							@error('kepemilikan')
								<div class="invalid-feedback">{{ $message }}</div>
						...
						<select name="kondisi" class="form-control @error('kondisi') is-invalid @enderror">
							<option value="">Pilih Kondisi</option>
								@foreach ($kategori_kondisi as $data)
									<option value="{{ $data->id }}" {{$data->id == $data_prasarana->kondisi ? 'selected' : ''}} >{{ $data->kondisi}}</option>
								@endforeach
						</select>
							@error('kondisi')
								<div class="invalid-feedback">{{ $message }}</div>
