		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Tambah Data Mahasiswa Asing</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('mahasiswa-asing.biodata.store') }}" method="post">
					@csrf
						...
						<select name="asal_id" id="seeAnotherFieldGroupProv" class="form-control @error('asal_id') is-invalid @enderror" readonly="readonly">
							<option value="">Pilih Asal</option>
								@foreach ($kategori_asal as $data)
									<option value="{{ $data->id }}" {{$data->id == 2 ? 'selected' : ''}} >{{ $data->asal}}</option>
								@endforeach
						</select>
							@error('asal_id')
								<div class="invalid-feedback">{{ $message }}</div>
						...
						<select name="jalur_masuk_id" disabled="disabled" class="form-control @error('jalur_masuk_id') is-invalid @enderror">
							<option value="">Pilih Jalur Masuk</option>
								@foreach ($kategori_jalur as $data)
									<option value="{{ $data->id }}" {{$data->id == 5 ? 'selected' : ''}} >{{ $data->jalur_masuk}}</option>
								@endforeach
						</select>
						...
						<select name="id_status" class="form-control @error('id_status') is-invalid @enderror" disabled>
							<option value="">Pilih Status Mahasiswa</option>
								...
								<option value="{{ $data->id }}" {{ $data->id == 3 ? 'selected' : '' }}>{{ $data->status}}</option>
								@endforeach
						</select>
						@error('id_status')
						<div class="invalid-feedback">{{ $message }}</div>
