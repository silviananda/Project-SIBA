		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel shadow-sm p-3 mb-5 bg-white rounded">
				<div class="x_title">
				    ...
				    <h2>Form Tambah Data Alumni</h2>
					<ul class="nav navbar-right panel_toolbox"></ul>
					<div class="clearfix"></div>
			    </div>

					<form role="form" action="{{ route('alumni.store') }}" method="post">
					@csrf
						...
						<div class="form-group">
							<label for="exampleFormControlInput1">Tanggal Lulus<span class="danger" style="color: #DC143C;">*</span></label>
								<input id="tahun_lulus" class="date-picker form-control @error('tahun_lulus') is-invalid @enderror" placeholder="dd-mm-yyyy" type="text" onfocus="this.type='date'" onclick="this.type='date'" onblur="this.type='text'" onmouseout="timeFunctionLong(this)" name="tahun_lulus" value="{{ old ('tahun_lulus') }}">
								<script>
									function timeFunctionLong(input) {
										setTimeout(function() {
											input.type = 'text';
						...
						<select name="id_mulai_kerja" class="form-control @error('id_mulai_kerja') is-invalid @enderror">
							<option value="">Pilih Status Mulai Kerja</option>
								@foreach ($mulai_kerja as $data)
									<option value="{{ $data->id }}" {{ old('id_mulai_kerja') == $data->id ? 'selected' : null }} >{{ $data->waktu_kerja}}</option>
								@endforeach
						</select>
						...
						<select name="id_jenis_pekerjaan" class="form-control @error('id_jenis_pekerjaan') is-invalid @enderror">
							<option value="">Pilih Jenis Pekerjaan</option>
								@foreach ($jenis_pekerjaan as $data)
									<option value="{{ $data->id }}" {{ old('id_jenis_pekerjaan') == $data->id ? 'selected' : null }} >{{ $data->jenis}}</option>
								@endforeach
						</select>
						...
						<select name="id_pendapatan" class="form-control @error('id_pendapatan') is-invalid @enderror">
							<option value="">Pilih Pendapatan</option>
								@foreach ($kategori_pendapatan as $data)
									<option value="{{ $data->id }}" {{ old('id_pendapatan') == $data->id ? 'selected' : null }} >{{ $data->pendapatan}}</option>
								@endforeach
						</select>
							@error('id_pendapatan')
								<div class="invalid-feedback">{{ $message }}</div>
						...
						<select name="id_waktu_tunggu" class="form-control @error('id_waktu_tunggu') is-invalid @enderror">
							<option value="">Pilih Waktu Tunggu</option>
								@foreach ($waktu_tunggu as $data)
									<option value="{{ $data->id }}" {{ old('id_waktu_tunggu') == $data->id ? 'selected' : null }} >{{ $data->waktu}}</option>
								@endforeach
						</select>
